...
public class MiLaucherActivity extends Activity {
    ...
    public void initData() {
        ...
        for (int i = 0; i < Configure.countPages; i++) {
            ...
            for (int j = PAGE_SIZE * i; j < (PAGE_SIZE * (i + 1) > lstDate.size() ? lstDate.size() : PAGE_SIZE
                    * (i + 1)); j++)
    ...
    public LinearLayout addGridView(int i) {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    if (getFristNonePosition(lists.get(ii)) < 0
                                        ...
                                        if (ii == Configure.countPages - 1
                                            ...
                                            ((DateAdapter) ((gridviews.get(lists.size() - 1)).getAdapter()))
                                                    .notifyDataSetChanged();
                                        }
                                    }
                                    /*
                                     * if(lists.get(Configure.countPages-1)!=null
                                     * &&lists.get(Configure.countPages-1).size()>0 &&
                                     * lists.get(Configure.countPages-1).
                                     * get(lists.get(Configure.countPages-1).size()-1)!=null){
                                     * lists.get(Configure.countPages-1).add(null); ((DateAdapter)
                                     * ((gridviews.get(Configure.countPages-1)).getAdapter())).notifyDataSetChanged(); }
                                     */
        ...
        gridView.setPageListener(new DragGrid.G_PageListener() {
            ...
            public void page(int cases, int page) {
                Log.v("test", "case:" + cases + "  page:" + page);
                switch (cases) {
                case 0:// 滑动页面
